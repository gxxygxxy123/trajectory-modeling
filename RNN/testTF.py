import torch
import pandas as pd
import os
import sys
import numpy as np
import math
import matplotlib.pyplot as plt
import seaborn as sns
import argparse
import warnings
from datetime import datetime
import pickle
import individual_TF

from threeDprojectTo2D import FitPlane, ProjectToPlane, ThreeDPlaneTo2DCoor, fit_3d, fit_2d, FitVerticalPlaneTo2D
from dataloader import RNNDataSet, PhysicsDataSet
from utils import predict
from utils.error_function import space_err, time_err, space_time_err, time_after_err
DIRNAME = os.path.dirname(os.path.abspath(__file__))
ROOTDIR = os.path.dirname(DIRNAME)
sys.path.append(f"{ROOTDIR}/lib")
from point import Point, load_points_from_csv, save_points_to_csv

parser = argparse.ArgumentParser(description="TF Testing Program")
parser.add_argument("-t","--time", type=float, help="Input Sequence Time (default 0.2)", default=0.2)
parser.add_argument("-w","--weight", type=str, help="TF Weight", required=True)
parser.add_argument("--folder", type=str, help="Test Folder", required=True)
parser.add_argument("--fps", type=float, required=True, help="Trajectories FPS")
parser.add_argument('--draw_predict', action="store_true", help = 'Draw predict dataset')
parser.add_argument('--no_show', action="store_true", help = 'No plt.show()')
args = parser.parse_args()

DXYT = True

# Argument
N = math.floor(args.time*args.fps)+1
TEST_DATASET = args.folder

# Device
device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')

# Dataset
test_dataset = RNNDataSet(dataset_path=TEST_DATASET, fps=args.fps, smooth_2d=True, smooth_3d=False)
#test_dataset = PhysicsDataSet(out_max_time=10.0, cut_under_ground=True, alpha=0.242, random='experiment', model='BLSTM')

trajectories_2d = test_dataset.whole_2d()
trajectories_3d = test_dataset.whole_3d()

# Evaluation
tf_space_2d = []
tf_space_time_2d = []
tf_space_3d = []
tf_space_time_3d = []
tf_time_2d = []
tf_time_after_2d = []
cost_time = []

weight_dict = torch.load(args.weight)

EMB_SIZE = weight_dict['emb_size']
HEADS = weight_dict['heads']
LAYERS = weight_dict['layers']
D_FF = weight_dict['d_ff']
mean = weight_dict['mean']
std = weight_dict['std']

model=individual_TF.IndividualTF(2, 3, 3, N=LAYERS,
                d_model=EMB_SIZE, d_ff=D_FF, h=HEADS, mean=mean,std=std).to(device)

model.load_state_dict(torch.load(args.weight)['state_dict'])
model.eval()

# Figure
fig, ax = plt.subplots()
ax.set_title("TF")

fig2, ax2 = plt.subplots()
ax2.set_title(f"[TF] Time After Error(2D) FPS:{args.fps}, t: {args.time}")
ax2.set_xlabel("Time(s)")
ax2.set_ylabel("Error(m)")

with torch.no_grad():
    for idx, traj_2d in trajectories_2d.items():
        if traj_2d.shape[0] <= N:
            continue

        inp = traj_2d[:N].copy()
        gt = traj_2d[N:].copy()

        t1 = datetime.now().timestamp()
        out = predict.predict2d_TF(inp, model, mean=mean, std=std, out_time=2.0, fps=args.fps, touch_ground_stop=True, device=device, dxyt=DXYT)
        cost_time.append(datetime.now().timestamp()-t1)

        if args.draw_predict:
            p = ax.plot(inp[:,0], inp[:,1], marker='o', markersize=1)
            ax.plot(gt[:,0], gt[:,1], color=p[0].get_color(), linestyle='--')
            ax.plot(out[inp.shape[0]:,0], out[inp.shape[0]:,1], marker='o', markersize=1, alpha=0.3, color=p[0].get_color())

        tf_space_2d.append(space_err(gt, out[inp.shape[0]:]))
        tf_space_time_2d.append(space_time_err(gt, out[inp.shape[0]:]))
        tf_time_2d.append(time_err(gt, out[inp.shape[0]:]))

        tmp = []
        for t in np.arange(0, 2, 0.02):
            tmp.append(time_after_err(traj_2d, out, t))
        tf_time_after_2d.append(tmp)

    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=RuntimeWarning)
        tf_time_after_2d = np.nanmean(np.array(tf_time_after_2d), axis=0)

    # for idx, traj_3d in trajectories_3d.items():
    #     if traj_3d.shape[0] <= N:
    #         continue

    #     inp = traj_3d[:N].copy()
    #     gt = traj_3d[N:].copy()

    #     out = predict.predict3d(inp, model, 'TF', mean, std, out_time=3.0, fps=args.fps, touch_ground_stop=True, device=device)

    #     tf_space_3d.append(space_err(gt, out[inp.shape[0]:]))
    #     tf_space_time_3d.append(space_time_err(gt, out[inp.shape[0]:]))


print(f"===TF===")
print(f"Weight: {args.weight}")
print(f"Space Error 2D (Average): {np.nanmean(tf_space_2d):.3f}m, std:{np.std(tf_space_2d):.3f}")
print(f"Space Time Error 2D (Average): {np.nanmean(tf_space_time_2d):.3f}m, std:{np.std(tf_space_time_2d):.3f}")
# print(f"Space Error 3D (Average): {np.nanmean(tf_space_3d):.3f}m")
# print(f"Space Time Error 3D (Average): {np.nanmean(tf_space_time_3d):.3f}m")
# print(f"Time Error (Average): {np.nanmean(tf_time_2d):.4f}s")
print(f"Cost Time (Average): {sum(cost_time)/len(cost_time):.4f}s")
print(f"Whole Data: {len(test_dataset.whole_2d())}, Used Data: {len(tf_space_time_2d)}")
ax2.plot(np.arange(0, 2, 0.02), tf_time_after_2d)
if not args.no_show:
    plt.show()


with open(f"{os.path.basename(args.weight)}.pkl", "wb") as fp:
    pickle.dump(tf_time_after_2d, fp)